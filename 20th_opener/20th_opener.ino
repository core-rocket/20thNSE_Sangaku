...
//global data
unsigned long time_from_liftoff_ms = 0;  //離床判定タイマー(燃焼終了検知)
...
//LED
#define PWM_LED_WHITE 20  //GPIO20を使用する
#define PWM_LED_BLUE 21   //GPIO21を使用する
#define RGB_LED_WHITE 16
#define RGB_LED_BLUE 17
#define RGB_LED_GREEN 25
// digitalWrite(PWM_LED_WHITE, HIGH);
...
void loop() {
  ...
  if (millis() - time_100Hz >= 10) {
    ...
    if (count_10Hz >= 10) {
      ...
      digitalWrite(PWM_LED_WHITE, HIGH);  //赤LED点灯
      Serial.println(downlink, BIN);
      CCP.uint32_to_device(CCP_downlink, downlink);
      digitalWrite(PWM_LED_WHITE, LOW);  //赤LED消灯
      count_10Hz = 0;
      count++;
      if (count >= 10) {
        //1Hz
  ...
  switch (nowphase) {
    case 1:
      if ((acceljudge_ground) || (altitudejudge_ground)) {
        ...
        time_from_liftoff_ms = millis();
      }
      break;
    case 2:
      if (millis() - time_from_liftoff_ms > maxaltime) {  //時間による頂点到達検知
        maxaltitude_data_judge_ms = true;
      }
      if (millis() - time_from_liftoff_ms > mecotime /*とりあえず燃焼時間5秒設定*/) {  //時間による燃焼終了検知
...
void reset() {
  ...
  open_accel_time = false;            //時間による燃焼終了判定&加速度による頂点到達
  open_altitude_time = false;         //時間による頂点到達&高度による頂点到達
  mecotime_data_judge_ms = false;     //燃焼終了検知
  maxaltitude_data_judge_ms = false;  //頂点到達検知
  time_from_liftoff_ms = 0;           //離床判定タイマー(燃焼終了検知)
  //テレメトリ
  downlink_STM_1 = 0;  //state transition model
  downlink_STM_2 = 0;
  downlink_outground_accel = 0;     //加速度による離床判定
  downlink_outground_altitude = 0;  //高度による離床判定
  downlink_meco_time = 0;           //時間による燃焼終了判定
  downlink_top_time = 0;            //時間による頂点到達
  downlink_open_accel = 0;          //加速度による頂点到達
  downlink_open_altitude = 0;       //高度による頂点到達
  downlink_pwm_1 = 0;               //サーボの現状確認(0,CLOSE   1,OPEN)
  downlink_pwm_2 = 0;               //サーボの現状確認(0,CLOSE   1,OPEN)
